            ...
            <div className="form-group form-group-full">
              ...
              {form.energy.gas.map((meter, idx) => (
                <div key={idx} className="meter-card gas">
                  ...
                  <div className="meter-grid">
                    <div className="form-group"><label>Zählertyp</label><select value={meter.zählertyp || "SLP"} onChange={(e) => setForm(p => ({ ...p, energy: { ...p.energy, gas: p.energy.gas.map((m, i) => i === idx ? { ...m, zählertyp: e.target.value } : m) } }))} disabled={loading}><option value="SLP">SLP</option><option value="RLM">RLM</option></select></div>
