...
function App() {
  ...
  const addLead = async (e) => {
    ...
    if (!form.phone.trim() || !form.email.trim() || !form.postalCode.trim()) {
      alert("Bitte Telefon, E-Mail und PLZ ausf├╝llen.");
      return;
    }
  ...
  const deleteLead = async (id) => {
    if (window.confirm("Lead wirklich l├Âschen?")) {
      try {
        await deleteDoc(doc(db, "leads", id));
      } catch (error) {
        console.error("Fehler beim L├Âschen:", error);
      }
    }
  };
  ...
  const handleFileUpload = (e) => {
    ...
    files.forEach((file) => {
      if (file.size > 10 * 1024 * 1024) {
        alert(`Datei ${file.name} ist zu gro├ƒ (max 10MB)`);
        return;
      }
    ...
    <div className="app">
      ...
      <header className="hero">
        <div className="hero-content" style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          ...
          <p className="subtitle energyo-subtitle">
            Leads erfassen, Nachfassen steuern, Abschl├╝sse im Blick behalten.
          </p>
        </div>
      ...
      <section className="stats-grid">
        ...
        <div className="card stat umsatz-card">
          ...
          <p>Ôé¼{stats.totalUmsatzPotential.toFixed(0)}</p>
        </div>
      </section>
      ...
      <section className="grid">
          ...
          <form onSubmit={addLead} className="form-grid">
            ...
            <label className="checkbox-label">
              <input
                ...
                disabled={loading}
              />
              B├╝ndelanfrage (mehrere Lieferstellen)
            </label>
            ...
            <div className="file-upload-section">
              <label htmlFor="file-input" className="file-upload-label">
                ­ƒôÄ Dateien hochladen (max 10MB)
              </label>
              <input
                id="file-input"
                type="file"
        ...
        <div className="card leads-card">
          ...
          <div className="search-filter-section">
            <input
              type="text"
              placeholder="­ƒöì Suche: Firma, Ansprechpartner, Telefon, E-Mail..."
              value={searchTerm}
            ...
            <div className="filter-controls">
              <select value={filterPriority} onChange={(e) => setFilterPriority(e.target.value)} className="filter-select">
                <option value="all">Alle Priorit├ñten</option>
                <option value="A">Priorit├ñt A</option>
                <option value="B">Priorit├ñt B</option>
                <option value="C">Priorit├ñt C</option>
              </select>
              ...
              <select value={filterCancellation} onChange={(e) => setFilterCancellation(e.target.value)} className="filter-select">
                <option value="all">K├╝ndigungsfenster: Alle</option>
          ...
          {filteredAndSortedLeads.length === 0 ? (
            <p className="no-leads">
              Keine Leads gefunden. {searchTerm && "Suchbegriff pr├╝fen."}
            </p>
          ) : (
                  ...
                  <div key={lead.id} className={`lead-item priority-${priority}`}>
                    <div className="lead-header">
                      ...
                      <div className="lead-badges">
                        ...
                        {hasCancellationWindow && <span className="badge alert">­ƒöö K├╝ndigungsfenster offen</span>}
                        {lead.contractEnd === "unknown" && <span className="badge warning">ÔØô Vertragsende unbekannt</span>}
                        {hasUnrealisticEnd && <span className="badge warning">ÔÜá´©Å Vertragsende vergangen</span>}
                        {isTodayDueNow && <span className="badge success">­ƒôà Heute f├ñllig</span>}
                        {isOverdueNow && <span className="badge danger">ÔÅ░ ├£berf├ñllig</span>}
                        {lead.bundleInquiry && <span className="badge info">­ƒôª B├╝ndelanfrage</span>}
                      </div>
                    </div>
                    ...
                    <div className="lead-umsatz">
                      ...
                      <span className="umsatz-value">
                        Ôé¼{umsatzPotential.toFixed(2)}
                        {lead.consumption && parseInt(lead.consumption) >= 50000 ? ` (${parseInt(lead.consumption)} kWh ├ù 0,01 Ôé¼)` : " (pauschal)"}
                      </span>
                    </div>
                    ...
                    <div className="lead-section">
                      <div className="lead-field">
                        ...
                        <div className="field-content">
                          <span>{lead.phone || "ÔÇö"}</span>
                          <span>{lead.email || "ÔÇö"}</span>
                        </div>
                      </div>
                      ...
                      <div className="lead-field">
                        ...
                        <div className="field-content">
                          <span>{lead.currentProvider || "ÔÇö"}</span>
                          <span>{lead.consumption ? `${lead.consumption} kWh` : "ÔÇö"}</span>
                        </div>
                      </div>
                      ...
                      <div className="lead-field">
                        ...
                        <div className="field-content">
                          <span>{lead.annualCosts ? `Ôé¼${parseInt(lead.annualCosts).toLocaleString("de-DE")}` : "ÔÇö"}</span>
                        </div>
                      </div>
                      ...
                      <div className="lead-field">
                        ...
                        <select value={lead.contractEnd} onChange={(e) => updateLeadField(lead.id, "contractEnd", e.target.value)} className="lead-select">
                          ...
                          <option value="">--- oder Datum w├ñhlen ---</option>
                    ...
                    {lead.attachments && lead.attachments.length > 0 && (
                      <div className="lead-attachments">
                        <label>­ƒôÄ Anh├ñnge ({lead.attachments.length})</label>
                          ...
                          {lead.attachments.map((att) => (
                            <div key={att.id} className="attachment-item-lead">
                              ...
                              <div className="attachment-actions">
                                ...
                                <button type="button" onClick={() => removeLeadAttachment(lead.id, att.id)} className="attachment-delete" title="L├Âschen">
                                  Ô£ò
                                </button>
                              </div>
                            </div>
                    ...
                    <div className="lead-file-upload">
                      <label htmlFor={`file-upload-${lead.id}`} className="file-upload-label-lead">
                        ­ƒôü Datei hochladen (Z├ñhlerfoto, Rechnung, etc.)
                      </label>
                      <input
                    ...
                    <div className="lead-comments">
                      ...
                      <div className="comment-input">
                        <input
                          type="text"
                          placeholder="Kommentar hinzuf├╝gen..."
